...
export default function AssetView() {
  ...
  const { qrCode, asset, event, location } = assetData;
  // Use GPS from QR code (primary source) or from asset
  const lat = location?.coordinates?.[1] || asset?.location?.coordinates?.[1];
  const lng = location?.coordinates?.[0] || asset?.location?.coordinates?.[0];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-lg mx-auto p-4 space-y-4">
        {/* Location/QR Info Card */}
        <div className="card">
          <div className="flex items-center mb-3">
            <FiMapPin className="text-primary-600 mr-2" size={20} />
            <h2 className="text-lg font-bold text-gray-800">{asset ? 'Asset Information' : 'Pole Location'}</h2>
          </div>
          <div className="space-y-2 text-sm">
            {/* QR/Pole info - always available */}
            <div className="flex justify-between">
              <span className="text-gray-500">Pole #:</span>
              <span className="font-bold text-primary-700">{qrCode?.poleNumber || asset?.poleNumber || code}</span>
            </div>
            <div className="flex justify-between">
            ...
            {location?.coordinates && (
              <div className="flex justify-between">
                ...
                <span className="font-mono text-xs">{location.coordinates[1]?.toFixed(6)}, {location.coordinates[0]?.toFixed(6)}</span>
              </div>
            )}
            
            {/* Asset details - only if QR is bound */}
            {asset && (
              <>
            ...
            {!asset && assetData.label && (
              <>
                {assetData.label.roadName && <div className="flex justify-between"><span className="text-gray-500">Road:</span><span className="font-medium">{assetData.label.roadName}</span></div>}
                {assetData.label.sectorName && <div className="flex justify-between"><span className="text-gray-500">Sector:</span><span className="font-medium">{assetData.label.sectorName}</span></div>}
                {assetData.label.zoneName && <div className="flex justify-between"><span className="text-gray-500">Zone:</span><span className="font-medium">{assetData.label.zoneName}</span></div>}
                {assetData.label.address && <div className="flex justify-between"><span className="text-gray-500">Address:</span><span className="font-medium">{assetData.label.address}</span></div>}
              </>
            )}
          </div>
        </div>
