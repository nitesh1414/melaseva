...
import { assetAPI, facilityAPI, complaintAPI, qrCodeAPI } from '../../services/api';
...
const createQRIcon = (bound) => L.divIcon({
  className: 'qr-marker',
  html: `<div style="background:${bound ? '#1d4ed8' : '#f59e0b'};width:18px;height:18px;border-radius:4px;border:2px solid white;box-shadow:0 0 4px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;">
    <span style="color:white;font-size:10px;font-weight:bold;">QR</span>
  </div>`,
  iconSize: [18, 18],
});
...
export default function AssetMap() {
  ...
  const [layers, setLayers] = useState({ assets: true, qrCodes: true, facilities: true, complaints: false });
  ...
  const { data: qrCodeMapData } = useQuery({
    queryKey: ['qrCodeMap', eventId],
    queryFn: () => qrCodeAPI.map(eventId).then(res => res.data.data),
    enabled: !!eventId && layers.qrCodes,
  });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <label className="flex items-center text-sm"><input type="checkbox" checked={layers.qrCodes} onChange={(e) => setLayers({...layers, qrCodes: e.target.checked})} className="mr-2" />
            <span className="inline-block w-3 h-3 bg-blue-600 rounded-sm mr-1"></span>QR Codes
          </label>
      ...
      <div className="card p-0 overflow-hidden" style={{ height: 'calc(100vh - 200px)' }}>
        <MapContainer center={[22.7196, 75.8577]} zoom={14} style={{ height: '100%', width: '100%' }}>
          ...
          {layers.qrCodes && (qrCodeMapData || []).map(qr => (
            <Marker key={qr.id} position={[qr.coordinates[1], qr.coordinates[0]]} icon={createQRIcon(qr.bindingStatus === 'BOUND')}>
              <Popup>
                <div className="text-sm">
                  <strong>QR: {qr.code}</strong><br/>
                  Pole #{qr.poleNumber}<br/>
                  Status: {qr.bindingStatus}<br/>
                  {qr.assetId && <>Asset: {qr.assetId}</>}
                  {qr.assetType && <><br/>Type: {qr.assetType?.replace(/_/g, ' ')}</>}
                </div>
              </Popup>
            </Marker>
          ))}
